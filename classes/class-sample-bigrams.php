...
(
	...
	function process_contents_using_HTML_Tag_Processor( $content ) {
		...
		while ( $p->next_token() ) {
			...
			switch ( $token_type ) {
				...
				case '#tag':
					$tag = $p->get_tag();
					///bw_trace2( $text, "tag", false );
					//$this->echo( $text );
					break;

				default:
					bw_trace2( $token_type, "token type", false );
			}
			if ( $text ) {
				$contents.=$text;
				$contents.=' ';
   ...
   */
	function sample_post( $post_ID, $post, $update ) {
		...
		// This has been wrong since June 2022 where it returned if the content didn't contain "Sampled from".
		// It should return if it does contain Sampled from.
		// However, once the content has been converted from a classic block to normal blocks
